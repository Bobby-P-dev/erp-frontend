...
const loadConfiguration = async () => {
    try {
        ...
        const res = await showApprovalConfiguration(configId)
        const d = res.data
        if (!d) throw new Error('Data konfigurasi tidak ditemukan.')

        form.value.document_type = d.document_type || 'purchase_requisition'
        form.value.company_id = d.company_id || null
        form.value.code = d.code || ''
        form.value.name = d.name || ''
        form.value.description = d.description || ''
        form.value.is_active = Boolean(d.is_active)
        form.value.applies_to_all_amounts = (d.min_amount === null && d.max_amount === null)
        form.value.min_amount = d.min_amount !== null ? String(d.min_amount) : ''
        form.value.max_amount = d.max_amount !== null ? String(d.max_amount) : ''
        form.value.levels = (d.levels || []).map((lvl, idx) => ({
            ...lvl,
            _uid: lvl._uid || `uid_${Date.now()}_${idx}`,
        }))
    } catch (err) {
        showError('Gagal!', 'Tidak dapat memuat konfigurasi alur persetujuan.', err)
...
const handleSubmit = async () => {
    ...
    try {
        ...
        showLoading('Memperbarui Konfigurasi...', 'Mohon tunggu sebentar.')
        ...
        await updateApprovalConfiguration(configId, payload)
        showSuccess('Berhasil!', 'Alur persetujuan berhasil diperbarui.')
        router.push({ name: 'admin.settings.approval' })
    } catch (error) {
        if (error.response?.status === 422 && error.response?.data?.errors) {
            ...
            showError('Gagal!', error.response?.data?.message || 'Terjadi kesalahan sistem saat memperbarui alur.', error)
        }
    } finally {
...
<template>
    <div class="space-y-6 max-w-5xl mx-auto pb-12">
        ...
        <PageHeader
            ...
            description="Perbarui aturan matriks persetujuan, batas nominal, atau tingkatan hierarki."
        >
            <template #icon>
        ...
        <!-- LOADING STATE -->
        <div v-if="isLoading" class="bg-white rounded-3xl p-12 border border-gray-100 shadow-sm text-center">
            <div class="inline-block animate-spin rounded-full h-8 w-8 border-2 border-indigo-600 border-t-transparent mb-2"></div>
            <p class="text-sm font-medium text-gray-500">Memuat data konfigurasi alur persetujuan...</p>
        </div>
